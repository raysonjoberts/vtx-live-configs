...
DEFAULT_REPO_USR = VTX_ROOT.parent / "repos" / "usr"
...
def render_report(install_usr: Path, repo_usr: Path, install_only: List[str], repo_only: List[str], changed: List[DiffInfo]) -> str:
    ...
    summary = [
        ...
        "## Install-only files",
        "",
        bullet_list(install_only),
        "## Repo-only files",
        "",
        bullet_list(repo_only),
...
def main(argv: List[str] | None = None) -> int:
    ...
    install_files = collect_files(install_usr)
    repo_files = collect_files(repo_usr)
