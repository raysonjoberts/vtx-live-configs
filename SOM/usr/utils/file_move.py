...
Other options:
  ...
  --skip-unchanged yes|no    # default: no (copy mode only; compare to dest)
  --is-intake-data yes|no    # default: no (copy mode only; compare to last pulled source)
...
STATE_DIR = os.path.join(BTDM_ROOT, "var", "state")
STATE_FILE = os.path.join(STATE_DIR, "file_move_index.json")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        choices=["yes", "no"],
        default="no",
        help="When mode=copy, skip if destination appears unchanged (compare dest file).",
    )
    ap.add_argument(
        "--is-intake-data",
        choices=["yes", "no"],
        default="no",
        help=(
            "When mode=copy, treat source as intake data: skip copy if the source "
            "size/mtime has not changed since last run, even if dest file is gone."
        ),
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    # Load state if we are in intake mode
    state: Dict[str, Any] = {}
    use_intake_state = args.mode == "copy" and args.is_intake_data == "yes"
    if use_intake_state:
    ...
    for src in sources:
        try:
            ...
            src_abs = src.resolve()
            dest_root_abs = dest_input.resolve()
            intake_key = f"{str(src_abs)}|{str(dest_root_abs)}"
            ...
            if args.convert_to:
                ...
                # Update intake state after a successful operation if enabled
                if use_intake_state:
                    try:
                        s = src.stat()
                        state[intake_key] = {"size": s.st_size, "mtime": int(s.st_mtime)}
                    except Exception as e:
                        logger.warning(f"Could not update intake state for {src}: {e}")
            else:
                # Normal move/copy without format conversion
                ensure_dir_for_target(target)

                if args.mode == "copy":
                    # Intake-mode skip: compare against last-seen source info
                    if use_intake_state:
                        try:
                            s = src.stat()
                            rec = state.get(intake_key)
                            if rec and rec.get("size") == s.st_size and int(rec.get("mtime", 0)) == int(s.st_mtime):
                                logger.info(f"Skipping intake copy (source unchanged): {src} -> {target}")
                                ok += 1
                                continue
                        except FileNotFoundError:
                            logger.warning(f"Source disappeared while checking: {src}")
                            continue

                    # Dest-based skip
                    if args.skip_unchanged == "yes" and target.exists():
                        if files_look_unchanged(src, target):
                            logger.info(f"Skipping unchanged copy (dest compare): {src} -> {target}")
                            ok += 1
                            # Still worth updating intake state so we remember we saw this version
                            if use_intake_state:
                                try:
                                    s = src.stat()
                                    state[intake_key] = {"size": s.st_size, "mtime": int(s.st_mtime)}
                ...
                # Update intake state after successful operation
                if use_intake_state:
                    try:
                        s = src.stat()
                    except FileNotFoundError:
                        # If we're in move mode, src may be gone; record nothing new
                        s = None
                    if s is not None:
                        state[intake_key] = {"size": s.st_size, "mtime": int(s.st_mtime)}
    ...
    # Persist state if we were in intake mode
    if use_intake_state:
