...
Behaviors:
- Rename by pointing --dest at a file path (works with --append-date).
- With --source-glob and multiple matches, --dest must be a directory.
- With --convert-to:
    * copy = convert and keep the source file
    * move = convert then delete the source file
- Date suffix is applied to the final base name before extension changes.
...
# BTDM paths
# ---------------------------------------------------------------------------
# Keep the standard BTDM_ROOT detection you use across scripts
...
def resolve_path(p: str) -> Path:
    """Resolve a possibly relative BTDM path to an absolute Path."""
    if not p:
    ...
    if pp.is_absolute():
        return pp
    return Path(BTDM_ROOT) / pp


def ensure_dir_for_target(target: Path) -> None:
    """Create parent directory for a file target, or the directory itself if target is a dir."""
    if target.suffix:
...
def add_date_suffix(p: Path, date_format: str) -> Path:
    """Insert _<date> before the extension, or append if no extension."""
...
def collect_sources(source: str | None, source_glob: str | None) -> List[Path]:
    ...
    # glob
    gp = resolve_path(source_glob)  # type: ignore[arg-type]
    # glob() does not error on no matches, we keep behavior controlled by --skip-missing
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    src.add_argument("--source", help="Path to a single source file (BTDM-relative or absolute)")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    # Determine if destination is a directory (explicit dir or path with no suffix)
    ...
    # Prepare destination directory (for dir or a single file)
    ensure_dir_for_target(dest_input)

    ok = 0
    for src in sources:
        try:
            # Compute target path (rename if dest is a file)
            if dest_is_dir:
                target = dest_input / src.name
            else:
                target = dest_input

            # Apply date suffix if requested
            if args.append_date == "yes":
                target = add_date_suffix(target, args.date_format)

            # Handle conversion vs raw copy/move
            if args.convert_to:
                # Set final extension according to conversion
                ...
                # Original behavior: move/copy the file as-is
                ensure_dir_for_target(target)
                if args.mode == "move":
