...
def load_config(config_path):
    ...
    config.read(os.path.join(CONFIG_DIR, "file_mover.conf"))  # updated config name
    return config
...
def build_routing_table(config):
    ...
    for pattern in config.sections():
        interval = config[pattern].get("interval", "hourly").strip().lower()
        if interval != "hourly":  # Only include hourly entries
            continue
