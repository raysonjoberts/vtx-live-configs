...
def match_heuristics(series, heuristics, global_config):
    ...
    stats = {
        ...
        "Heuristic Match Ratio": match_ratio,
        "Heuristic Unique Ratio": unique_ratio,
        "Heuristic Viable Ratio": viable_ratio
    }
...
def main():
    ...
    detailed_rows = []
    required_attrs = [c for c in criteria if c.get("required")]

    for col in df.columns:
        row = {"Field Name": col}
        all_values = df[col].dropna().astype(str)
        for crit in required_attrs:
            attr = crit["attribute"]
            global_heuristic_config = {
                "min_match_ratio": crit.get("min_match_ratio"),
                "min_unique_ratio": crit.get("min_unique_ratio"),
            ...
            _, stats = match_heuristics(all_values, crit.get("value_heuristics", []), global_heuristic_config)
            row[f"{attr} Match Ratio"] = round(stats.get("Heuristic Match Ratio", 0.0) * 100, 2)
            row[f"{attr} Unique Ratio"] = round(stats.get("Heuristic Unique Ratio", 0.0) * 100, 2)
            row[f"{attr} Viable Ratio"] = round(stats.get("Heuristic Viable Ratio", 0.0) * 100, 2)
        detailed_rows.append(row)

    detailed_df = pd.DataFrame(detailed_rows)
    detailed_df.to_csv(r"C:\BTDM_7.1\var\analysis\detailed_analysis.csv", index=False)
    print("Saved detailed analysis to C:\BTDM_7.1\var\analysis\detailed_analysis.csv")
