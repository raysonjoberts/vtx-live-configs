...
def match_value_examples(series: pd.Series, example_config):
    try:
        ...
        exact_match = example_config.get("exact_token_match", False)
        ...
        if string_series.empty or not example_values:
            return False
        ...
        print(f"Example match ratio for column: {match_ratio:.2f}")
    ...
    for rule in heuristics:
        ...
        if rule["type"] == "string":
            ...
            if "pattern" in rule:
                ...
                min_ratio = rule.get("min_match_ratio", 0.5)
