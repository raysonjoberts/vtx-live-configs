...
def match_heuristics(series, heuristics):
    ...
    for rule in heuristics:
        ...
        if "min_viable_ratio" in rule:
            viable_ratio = string_series.count() / len(series)
            if viable_ratio < rule["min_viable_ratio"]:
                matched = False

        if "max_unique_ratio" in rule:
            max_unique_ratio = string_series.nunique() / len(string_series)
            if max_unique_ratio > rule["max_unique_ratio"]:
                matched = False
...
def match_value_examples(series, example_config):
    ...
    try:
        ...
        example_values = set(pd.read_csv(example_config["path"])[example_config["column"]]
                             .dropna().astype(str).str.lower().str.strip())

        pattern = re.compile(r"\b(" + "|".join(re.escape(name) for name in example_values) + r")\b", re.IGNORECASE)
        string_series = series.dropna().astype(str).str.lower().str.strip()

        match_ratio = string_series.apply(lambda v: bool(pattern.search(v))).mean()
