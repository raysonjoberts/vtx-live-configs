        ...
        for crit in criteria_list:
            ...
            if should_declare_match(name_match, heuristic_match, example_match, requirement):
                result = {
                    ...
                    "Value Match Ratio": round(match_ratio * 100, 2) if "match_ratio" in locals() else None,
                    "Unique Ratio": round(unique_ratio * 100, 2) if "unique_ratio" in locals() else None,
                    "Max Length Violation Ratio": round(too_long * 100, 2) if "too_long" in locals() else None,
                    "Min Length Violation Ratio": round(too_short * 100, 2) if "too_short" in locals() else None
                }
                    "Field Name": col,
                    "Program Attribute": attr,
                    "Match Type": "+".join([k for k, v in {"name": name_match, "value": heuristic_match, "example": example_match}.items() if v]),
