...
DEFAULT_INPUT = VTX_ROOT / "usr" / "config" / "default" / "master_data_dictionary.yaml"
DEFAULT_OUTPUT_DIR = VTX_ROOT / "var" / "dictionaries" / "framework"
...
def render_index(program_pages: List[Dict[str, Any]]) -> str:
    ...
    for heading, names in INDEX_SECTIONS:
        ...
        if links:
            boxes.append(
                ...
                f'<div class="link-list">{"".join(links)}</div>'
                '</div></section>'
            )
    ...
    return shell_head(
        ...
        [("Home", "index.html")],
        [("Attribute Summary", "attribute_summary.html", "primary"), ("Sources Summary", "authoritative_source_summary.html", "alt")],
    ) + (
...
def render_domain_page(page: Dict[str, Any]) -> str:
    sections: List[str] = [
    ...
    return shell_head(
        page['program'],
        f'Attributes used by {page["program"]}. Search the full dictionary text below.',
        [("Search", "#search"), ("Index", "index.html")],
    ) + ''.join(sections) + script_block('.detail-item', 'attributes') + footer_block('Metadata Model - Focused Domain View')
...
def write_site(output_dir: Path, program_pages: List[Dict[str, Any]], attribute_items: List[Dict[str, Any]], source_items: List[Dict[str, Any]]) -> List[Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    pages = {
        'index.html': render_index(program_pages),
    ...
    for page in program_pages:
        pages[page['slug']] = render_domain_page(page)
    written: List[Path] = []
...
def main() -> int:
    ...
    written = write_site(DEFAULT_OUTPUT_DIR, program_pages, attribute_items, source_items)
