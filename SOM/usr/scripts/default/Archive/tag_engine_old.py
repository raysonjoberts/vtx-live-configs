...
def run_job(job: Dict[str, Any]) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        # Precompute value lists for all fields used by any tag
        field_values: Dict[str, List[str]] = {}
        for field in all_fields:
            field_values[field] = split_values(row.get(field), key_delimiter)
        ...
        for tag in tags:
            ...
            for field in tag_fields:
                max_len = max(max_len, len(field_values.get(field, [])))
            if max_len < 1:
                max_len = 1
            ...
            for field in tag_fields:
                ...
                tag_values[field] = align_values(vals, max_len)

            # Build groups by key_fields (unique key tuples)
            groups: Dict[Tuple[str, ...], List[int]] = {}
            if not context_fields:
                groups[()] = list(range(max_len))
            else:
                for idx in range(max_len):
                    key_tuple = tuple(tag_values[k][idx] for k in context_fields)
            ...
            for key_tuple, indices in groups.items():
                ...
                for rule in tag.get("rules", []) or []:
                    ...
                    for cond in conditions:
                        ...
                        values_list = tag_values.get(field, [])
                        group_values = [values_list[i] for i in indices]
                    ...
                    for match_field in match_field_names:
                        row_out[match_field] = ""
                    context_map = {context_fields[i]: key_val for i, key_val in enumerate(key_tuple)}
                    for key_field in key_field_names:
