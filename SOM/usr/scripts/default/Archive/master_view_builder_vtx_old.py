...
logger = get_logger("master_view_builder_vtx")
...
def expand_view(
    ...
    for _, row in df.iterrows():
        ...
        expand_values = arrays.get(expand_by, key_values_raw)
        key_order: List[str] = []
        for key in expand_values:
            if key not in key_order:
                key_order.append(key)

        for key in key_order:
            positions = [idx for idx, value in enumerate(expand_values) if value == key]
            record: Dict[str, str] = {}
            ...
            for group in resolved_groups:
                for field in group:
                    if field == expand_by:
                        record[field] = key
                        continue
                    values = [arrays[field][pos] for pos in positions] if positions else []
                    record[field] = delimiter.join(values) if values else ""
