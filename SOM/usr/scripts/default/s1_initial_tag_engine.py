...
DEFAULT_CONFIG_PATH = VTX_ROOT / "usr" / "config" / "run" / "s1_initial_tag_engine.yaml"
...
INVALID_REGEX_WARNINGS: set[str] = set()
...
def compare_list_values(values: List[str], operator: str, target: Any) -> bool:
    ...
    if operator == "regex":
        pattern = normalize_text(target)
        if not pattern:
            return False
        try:
            compiled = re.compile(pattern)
        except re.error as exc:
            if pattern not in INVALID_REGEX_WARNINGS:
                logger.warning("Invalid regex pattern '%s': %s", pattern, exc)
                INVALID_REGEX_WARNINGS.add(pattern)
            return False
        return any(compiled.search(v) is not None for v in values)
